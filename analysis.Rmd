...
cog_plot <- cog_plot[,1:2]
cog_plot_long <- gather(cog_plot, key = "variable", value = "value")
cog_plot_long$ID <- as.factor(rep(df$id,2)) # add participant IDs
cog_plot_long$variable <- as.factor(ifelse(cog_plot_long$variable == 'cog_load_raw_TLX_score_own',
                                 'Non-AI writing session',
                                 'AI writing session'))

# plot
ggplot(cog_plot_long,aes(x = factor(variable, levels = c('Non-AI writing session', 'AI writing session')), y = value, fill = variable)) +
  ...
  geom_line(aes(group=ID, color=ID), size = 1.2) +
  scale_color_manual(values = participant_colors) +
  scale_y_continuous(limits=c(1,6),breaks=seq(1,6,1)) +
  labs(title = "Difference in cognitive load between writing sessions", x = "", y = "Raw unweighted cognitive load (average of 4 TLX items, Very low - Very high)") +
  theme_classic() +
...
suggestions_plot <- suggestions_plot %>% 
  mutate(criteria = recode(name,
                ...
                cal_matches_tense = 'Are suggestions grammatically correct?'
                ))
...
#add option to include it even if it had 0 responses
#useful_when_study$Var1 <- as.character(useful_when_study$Var1)
#useful_when_study <- rbind(useful_when_study, c("At the end of writing the story", 0))
#useful_when_study$Var1 <- as.factor(useful_when_study$Var1)
...
p_when_stu <- ggplot(useful_when_study, aes(x = reorder(Var1, Freq), y = Freq)) +
  geom_bar(width = 0.3, stat = "identity", aes(fill = Var1)) +
  scale_fill_manual(values=c('#65e0bd', "#0686bd", '#bdaf59', '#013f59')) +
